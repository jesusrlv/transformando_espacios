...
while($rowCategoria=$resultadoCategorias->fetch_assoc()){
    ...
    //SELECT usr.id as id, usr.nombre as nombre, usr.curp as curp, usr.edad as edad, usr.municipio as municipio, usr.telefono as telefono, AVG(calificacion.calificacion) as calificacion FROM usr INNER JOIN calificacion ON usr.id = calificacion.id_ext WHERE categoria = 1 AND perfil = 1 GROUP BY id ORDER BY calificacion DESC; 
    // while interno
    $sqlUsr = "SELECT usr.id as id, usr.nombre as nombre, usr.curp as curp, usr.edad as edad, usr.municipio as municipio, usr.telefono as telefono, (SUM(calificacion.calificacion)/15) as promedio FROM usr INNER JOIN calificacion ON usr.id = calificacion.id_ext WHERE categoria = '$categoria' AND perfil = 1 GROUP BY id ORDER BY promedio DESC";
    ...
    while($rowUsr=$resultadoUsr->fetch_assoc()){
        ...
        echo'
        <td><span class="badge text-bg-primary">'.round($rowUsr['promedio'],PHP_ROUND_HALF_DOWN).'</span></td>
        ';
        echo'</tr>
        ';
        echo'
